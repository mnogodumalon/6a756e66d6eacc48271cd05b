...
import type { Kundenverwaltung, Fahrerverwaltung, Bestellverwaltung, CreateKundenverwaltung, CreateFahrerverwaltung, CreateBestellverwaltung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getKundenverwaltungEntry(id: string): Promise<Kundenverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Kundenverwaltung;
    return enrichLookupFields([record], 'kundenverwaltung')[0];
  }
  static async createKundenverwaltungEntry(fields: CreateKundenverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.KUNDENVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'kundenverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateKundenverwaltungEntry(id: string, fields: Partial<CreateKundenverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.KUNDENVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kundenverwaltung') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- FAHRERVERWALTUNG ---
  static async getFahrerverwaltung(): Promise<Fahrerverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FAHRERVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Fahrerverwaltung[];
    return enrichLookupFields(records, 'fahrerverwaltung');
  }
  static async getFahrerverwaltungEntry(id: string): Promise<Fahrerverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FAHRERVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Fahrerverwaltung;
    return enrichLookupFields([record], 'fahrerverwaltung')[0];
  }
  static async createFahrerverwaltungEntry(fields: CreateFahrerverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.FAHRERVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'fahrerverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateFahrerverwaltungEntry(id: string, fields: Partial<CreateFahrerverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.FAHRERVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'fahrerverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async deleteFahrerverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FAHRERVERWALTUNG}/records/${id}`);
  }

  // --- BESTELLVERWALTUNG ---
  static async getBestellverwaltung(): Promise<Bestellverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BESTELLVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Bestellverwaltung[];
    return enrichLookupFields(records, 'bestellverwaltung');
  }
  static async getBestellverwaltungEntry(id: string): Promise<Bestellverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BESTELLVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Bestellverwaltung;
    return enrichLookupFields([record], 'bestellverwaltung')[0];
  }
  static async createBestellverwaltungEntry(fields: CreateBestellverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.BESTELLVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'bestellverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateBestellverwaltungEntry(id: string, fields: Partial<CreateBestellverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.BESTELLVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bestellverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async deleteBestellverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BESTELLVERWALTUNG}/records/${id}`);
  }
