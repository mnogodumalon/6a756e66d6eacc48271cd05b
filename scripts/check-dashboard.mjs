...
// names, codes) take an /* i18n-exempt */ on the same line.
{
  const literalHits = [];
  const jsxText = />[^<>{}\n]*[A-Za-zÄÖÜäöüßÀ-ž]{3,}[^<>{}\n]*</;
  const attrText = /\b(?:title|placeholder|label|aria-label|alt|emptyLabel|emptyText)=(?:"[^"{}]*[A-Za-zÄÖÜäöüßÀ-ž]{3,}[^"{}]*"|'[^'{}]*[A-Za-zÄÖÜäöüßÀ-ž]{3,}[^'{}]*')/;
  for (let i = 0; i < dashLines.length; i++) {
    const l = dashLines[i];
    if (l.includes('i18n-exempt')) continue;
    const trimmed = l.trim();
    if (trimmed.startsWith('//') || trimmed.startsWith('*') || trimmed.startsWith('/*')) continue;
    if (jsxText.test(l) || attrText.test(l)) literalHits.push(`    line ${i + 1}: ${l}`);
  }
