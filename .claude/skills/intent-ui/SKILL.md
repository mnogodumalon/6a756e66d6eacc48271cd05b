...
**Draft persistence: wire `draftKey` + `draft` + `onDraftRestore`, and give the SAME key to
`useIntentSubmit`.** Keep ALL wizard answers in ONE state object, pass it as `draft`; the shell
saves it (with the step) on every change and restores both on the next visit — an interruption
(phone call, dead spot, reload) costs nothing, and a shared `?step=N` link lands on populated steps
instead of blank ones. The hook clears the draft on success; a draft that survives its flow
resurrects a finished wizard. The key is a plain `intent:<slug>` string — the shell namespaces it
per dashboard itself, do not add ids to it.
...
// ✅ one draft object, saved+restored by the shell; cleared on success by the hook
const [state, setState] = useState({ eventId: '', gaeste: [] as string[] });
<IntentWizardShell draftKey="intent:event-vorbereiten" draft={state}
  onDraftRestore={(d) => setState(d as typeof state)} … >
const { submit, … } = useIntentSubmit(async () => { … }, { draftKey: 'intent:event-vorbereiten' });
```
...
write. `check-intents` fails a flow page without `<SummaryStep>`. It is its
OWN step in the `steps` array (label it "Prüfen") — never rendered below a
still-editable form: the button above it then promises a step change that
never happens, and the same data is on screen twice.

```tsx
import { SummaryStep } from '@/components/blocks/SummaryStep';
import { useIntentSubmit } from '@/hooks/useIntentSubmit';

const { submit, submitting, error, done, result } = useIntentSubmit(async () => {
  const auftrag = await LivingAppsService.createAuftraegeEntry({ nummer, … });
  return { auftrag, nummer };  // ← render the SuccessStep from THIS
}, { draftKey: 'intent:auftrag-anlegen' });   // hook clears the draft on success
...
{step === 3 && !done && (
  <SummaryStep
    ...
    missing={[
      ...(state.beschreibung.trim() ? [] : ['Arbeitsbeschreibung']),
    ]}
    error={error}
  />
)}
...
- `missing` — REQUIRED: labels of the brief's `!` fields that are still empty, `[]` when all are
  filled. Non-empty disables confirm and names what is missing. The API accepts empty required
  fields without complaint — this prop is the only enforcement there is. A live flow shipped an
  order with an empty required description because nothing computed this list.
...
**Render the success step from the submit RESULT — never from refetched lists or live-derived
values.** Your write just changed the data: the closed order vanished from the eligible list, and a
recomputed running number counts the record you created. Both live-seen: a flow ended on a BLANK
screen because its success body was gated on the refetched selection, and a revisited flow announced
an order number that was never written.

```tsx
// ❌ gone after the write — auftraege was refetched, the order is no longer 'offen'
{step === 4 && selectedAuftrag && <SuccessStep title={`${selectedAuftrag.fields.nummer} abgeschlossen`} … />}

// ✅ the submit callback returned everything the success step names
{step === 4 && result && <SuccessStep title={`Auftrag ${result.nummer} angelegt`} … />}
```

Same reason: the `steps` array passed to the shell is a CONSTANT. A steps list derived from data
loses entries when the write changes that data — the step circles visibly collapsed mid-flow.

### useIntentSubmit — submit state for the final write

```tsx
import { useIntentSubmit } from '@/hooks/useIntentSubmit';
const { submit, submitting, error, done, result, reset } =
  useIntentSubmit(async () => { …writes…; return data; }, { draftKey: 'intent:<slug>' });
```

Wraps the flow's write callback and owns the rules every flow needs:
...
the success transition (`done` flips / `submit()` resolves truthy → show
the SuccessStep), and — when you pass the SAME `draftKey` you gave the shell —
clearing the saved draft on success, so the next visit starts fresh instead of
resurrecting a finished wizard. `reset` backs a "weiteren Auftrag anlegen"
action. Chained creates stay inside the ONE callback — store each `record_id`
as you go (idempotency, see below). Return everything the SuccessStep will
name (the number, the counts) — `result` is its only safe data source.
...
### 5. Reversible actions get undo, not a confirm dialog (gate-enforced)
...
irreversible writes, and then it names the consequence. `check-intents` fails
a page that has an "entfernen" action but never calls `undoToast`.
